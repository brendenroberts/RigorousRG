...
int main(int argc, char *argv[]) {
    ...
    ss << "_N" << std::setw(2) << N << "_seed" << std::setw(10) << seed;
    auto id = ss.str();
    ...
    for(auto a : args(hsps.front())) {
        ...
        for(auto i : range(p)) {
            ...
            for(auto j : range1(n))
                if(i/int(pow(2,j-1))%2 == 1) istate.set(j,"Occ");
            auto st = MPS(istate);
            V.push_back(st);
            }
    ...
    for(auto i : range(nTEBD)) {
        res = applyMPO(K,res,{"Cutoff",eps,"MaxDim",MAX_BOND,"Nsweep",20});
        ...
        if(i%10 == 0 || i == nTEBD-1) {
            ...
            for(auto q : range(nblock(extIndex)))
                if(q == 0)
                    std::cout << std::fixed << std::setprecision(14) << eGS << " (q:" << q << " gap:" 
                              << std::scientific << std::setprecision(4) << -S.elt(2,2)-eGS << ") ";
                else
                    std::cout << std::scientific << std::setprecision(6) << -S.elt((int)s*q+1,(int)s*q+1)-eGS << " (q:" << q << " gap:"
                              << std::scientific << std::setprecision(4) << -S.elt((int)s*q+2,(int)s*q+2)+S.elt((int)s*q+1,(int)s*q+1) << ") ";
            std::cout << std::endl; 
            }
        }
    ...
    for(int q : range(nblock(extIndex))) {
        ...
        for(auto i : range1(s)) {
            auto fc = MPS(res);
            fc.ref(eSite) *= setElt(dag(extIndex)(s*q+i));
    ...
    do {
        ...
        eGS = eigenspace.front().front().first;
        minGap = eigenspace.front().at(1).first-eGS; 
        for(auto q : args(eigenspace))
            if(q == 0)
                std::cout << std::fixed << std::setprecision(14) << eGS << " (q:" << q << " gap:" 
                          << std::scientific << std::setprecision(4) << eigenspace.at(q).at(1).first-eGS << " m:"
        ...
        std::cout << std::endl;
    } while(convGS > std::max(epx,std::min(eps,minGap*5e-3)) && count < nDMRG);
    ...
    dbEntry.fill('0');
    for(auto q : args(eigenspace)) {
        dbEntry << "# N s D t M seed q E0 ..." << std::endl;
        ...
        for(auto i : range(s)) {
            dbEntry << std::setprecision(16) << sector.at(i).first << " ";
            
            std::ostringstream().swap(ss);
            ss << id << "_q" << q << "_e" << i << ".dat";
            auto stateFilename = ss.str();
            writeToFile(stateFilename,sector.at(i).second);
            }
        dbEntry << std::endl;
        }
